...
//
//Функция GetFileSizeEx используется для получения размера файла.Вот параметры этой функции :
//
//of: Дескриптор файла(HANDLE), который был открыт с использованием функции CreateFile.
//
//fileSize : Указатель на структуру LARGE_INTEGER, куда будет записан размер файла.

//
//Функция SetFilePointer устанавливает текущую позицию в файле.Вот параметры этой функции :
//
//of: Дескриптор файла(HANDLE), который был открыт с использованием функции CreateFile.
//
//lDistanceToMove : Относительное смещение, которое будет добавлено или вычтено из текущей позиции в файле.Этот параметр может быть положительным, отрицательным или нулевым.
//
//lpDistanceToMoveHigh : Указатель на переменную, в которую будет записано значение старших битов lDistanceToMove.Если этот параметр равен NULL, старшие биты игнорируются.
//
//dwMoveMethod : Способ перемещения, который определяет, как интерпретировать lDistanceToMove.Это может быть одно из следующих значений :
//
//FILE_BEGIN: Начало файла.lDistanceToMove трактуется как относительное смещение от начала файла.
//FILE_CURRENT : Текущая позиция.lDistanceToMove трактуется как относительное смещение от текущей позиции в файле.
//FILE_END : Конец файла.lDistanceToMove трактуется как относительное смещение от конца файла.